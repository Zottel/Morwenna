z0ttel = function(role, home_x, home_y)
	...
	roles.probe = function()
		...
		search_planets = function() -- {
			neighbour_ships = get_entities(5000, SHIP)
			if #neighbour_ships > 5 then
				random_search()
				return
			end

			if next_planet ~= nil and get_distance(next_planet) < 100 then
				if colonize(next_planet) then
					next_planet = nil
					return
				else
					next_planet = nil
			...
			next_planet = nil
			for _, v in pairs(get_entities(100000, PLANET))do
				if get_player(v) == 0 and visited_planets[v] == nil and get_distance(v) < max_dist then
					next_planet = v
